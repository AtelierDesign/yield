      ...
      <Menu open onOpenChange={() => {}}>
        <MenuContent>
          ...
          {checkboxItems.map(({ label, state: [checked, setChecked], disabled }) => (
            <MenuCheckboxItem key={label} checked={checked} onCheckedChange={setChecked} disabled={disabled}>
              {label}
            </MenuCheckboxItem>
          ))}
        </MenuContent>
...
const RadioItemsMenu = () => {
  ...
  return (
    <>
      <Box ref={ref} css={{ width: 150, mt: 50, mb: 400 }} />
      <Menu open modal>
        <MenuContent onCloseAutoFocus={event => event.preventDefault()} avoidCollisions={false} align="start">
